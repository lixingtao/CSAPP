...
static void *find_fit(size_t asize){
	void *bp;
	void *fitp=fit_listp;
	for(bp=fit_listp; GET_SIZE(HDRP(bp))>0 ; bp=NEXT_BLKP(bp)){
		if(!GET_ALLOC(HDRP(bp)) && GET_SIZE(HDRP(bp))>=asize){
			fit_listp=NEXT_BLKP(bp);
			return bp;
		}
	}
	for(bp=heap_listp;bp!=fit_listp && GET_SIZE(HDRP(bp))>0; bp=NEXT_BLKP(bp)){
		if(!GET_ALLOC(HDRP(bp)) && GET_SIZE(HDRP(bp))>=asize){
			fit_listp=NEXT_BLKP(bp);
			return bp;
		}
